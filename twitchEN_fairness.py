...
def detect_fluid_communities(graph):
    ...
    partition = nx.community.asyn_fluidc(graph, k=10)
    partition = list(partition)
    return partition


def get_dataset():
    filename = "twitch/ENGB/musae_ENGB_edges.csv"
...
def load_attributes():
    attributes_file = pd.read_csv("twitch/ENGB/musae_ENGB_target.csv")
...
def count_red_and_blue_nodes():
    ...
    with open("twitch/ENGB/musae_ENGB_target.csv", "r") as csvfile:
...
def get_communities_degrees(dataset, communities):
    ...
    for index, community in enumerate(communities):
        community_degrees = {}
        communitySubgraph = dataset.subgraph(community)
        for node in communitySubgraph.nodes():
            community_degrees[node] = communitySubgraph.degree(node)
...
def improve_balance_fairness(communities):
    ...
    for community in communities_copy:
        new_dictionary[str(community)] = calculate_balance_fairness(community, red_nodes)
