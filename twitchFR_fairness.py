...
def detect_fluid_communities(graph):
    ...
    partition = nx.community.asyn_fluidc(graph, k=8)
    partition = list(partition)
    return partition


def get_dataset():
    filename = "twitch/FR/musae_FR_edges.csv"
    dataset = nx.read_edgelist(filename, delimiter=',', nodetype=int)

    con_component = max(nx.connected_components(dataset), key=len)
    twitch_largest_component = dataset.subgraph(con_component)

    dataset = twitch_largest_component
    return dataset

def load_attributes():
    attributes_file = pd.read_csv("twitch/FR/musae_FR_target.csv")
    dict_of_attributes = dict(zip(attributes_file["new_id"], attributes_file["mature"]))
...
def count_red_and_blue_nodes():
    red = 0
    blue = 0
    # count total number of each attribute (1 for females, 0 for males)
    with open("twitch/FR/musae_FR_target.csv", "r") as csvfile:
        csv_reader = csv.reader(csvfile)
        next(csv_reader) # skip the header row
        for row in csv_reader:
            if row[2] == 'True': # red nodes
                red += 1
            else: # blue nodes
                blue += 1
...
def count_attributes_communities(communities):
    ...
    for community in communities:
        ...
        for node in community:
            if dict_of_attributes[node] == True:
                red_counter += 1
            elif dict_of_attributes[node] == False:
                blue_counter += 1
...
def count_community_balance_fairness(communities):
    community_fairness = {}
    i = 0
    fairness_balance = 0
    mean_fairness = 0
    ...
    for community, blue_counter in community_blue.items():
        ...
        mean_fairness += fairness_balance
        i += 1
...
#returns blue and red nodes with low degree from a community
...
def split_communities_by_fairness(community_fairness):
    positive_fairness = {}
    negative_fairness = {}
    positive_fairness_list = []
    negative_fairness_list = []
    ...
    for community, fairness in sorted_positive_fairness.items():
        ...
        positive_fairness_list.append(c)

    for community, fairness in sorted_negative_fairness.items():
        c = list(eval(community))
        negative_fairness_list.append(c)
    return positive_fairness_list, negative_fairness_list
...
def seperate_nodes(community):
    ...
    for node in community:
        if dict_of_attributes[node] == True: # then the node is red
...
# TODO THESE 2 ONES ARE THE SAME
...
def improve_balance_fairness(communities):
    mean_fairness, fairness_dict = count_community_balance_fairness(communities)
    print("Old mean balance fairness: ", mean_fairness)
    positive_fairness_list, negative_fairness_list = split_communities_by_fairness(fairness_dict)
    communities_copy = [list(community) for community in communities]
    communities_copy = [sorted(sublist) for sublist in communities_copy] #convert them in a list in order to have easier access
    red_nodes, blue_nodes = get_red_and_blue_nodes(communities_copy)

    for community in negative_fairness_list:
        ...
        communities_copy, positive_fairness_list = exchange_nodes(community, positive_fairness_list, blue_nodes_low_degree, communities_copy)

        new_modularity = nx.community.modularity(dataset, communities_copy)
        if (new_modularity <= (2*initial_modularity)/3):
            break

    new_fairness_dictionary = {}
    for community in communities_copy:
        new_fairness_dictionary[str(community)] = calculate_balance_fairness(community, red_nodes)

    new_mean_fairness = return_mean_fairness(new_fairness_dictionary)
    print("New mean balance fairness: ", new_mean_fairness)

    return new_fairness_dictionary, communities_copy
...
def calculate_balance_fairness(community, red_nodes):
...
def improve_modularity_fairness(communities):
    ...
    positive_fairness_list, negative_fairness_list = split_communities_by_fairness(fairness_modularity)
    communities_copy = [list(community) for community in communities]
    ...
    for community in negative_fairness_list:
        blue_nodes_in_community, red_nodes_in_community = seperate_nodes(community)
        blue_nodes_low_degree, red_nodes_low_degree = get_low_degree_attributes(blue_nodes_in_community, red_nodes_in_community)
        communities_copy, positive_fairness_list = exchange_nodes_for_modularity(community, positive_fairness_list, blue_nodes_low_degree, communities_copy,red_nodes,blue_nodes)
...
def find_best_modularity(red_node, target_communities, red_nodes, blue_nodes):
    best_modularity = 100
    best_target = target_communities[0]

    for target_community in target_communities:
        target_community.append(red_node)
        ...
        target_community.remove(red_node)
