...
def get_dataset():
    filename = "facebook_dataset/facebook_combined.txt/facebook_combined.txt"
    dataset = nx.read_edgelist(filename, delimiter=' ', nodetype=int)
    con_component = max(nx.connected_components(dataset), key=len)
    github_largest_component = dataset.subgraph(con_component)
    dataset = github_largest_component
    return dataset


def load_attributes():
    attributes_file = pd.read_csv("facebook_dataset/target.csv")
    dict_of_attributes = dict(zip(attributes_file["node"], attributes_file["gender"]))
...
def count_red_and_blue_nodes():
    red = 0
    blue = 0
    # count total number of each attribute (1 for red, 0 for blue)
    with open("facebook_dataset/target.csv", "r") as csvfile:
...
def count_attributes_communities(communities):
    ...
    for community in communities:
        ...
        for node in community:
            if dict_of_attributes[node] == 1:
                red_counter += 1
            elif dict_of_attributes[node] == 0:
                blue_counter += 1
...
def seperate_nodes(community):
    ...
    for node in community:
        if dict_of_attributes[node] == 1: # then the node is red
...
def improve_modularity_fairness(communities):
    ...
    new_mean_modularity = return_mean_fairness(new_modularity)
    print("New mean modularity fairness: ", new_mean_modularity)
    return new_mean_modularity, communities_copy
